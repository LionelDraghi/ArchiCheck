...
private package Archicheck.IO is
   ...
   -- Function: GNU_Prefix
   -- -------------------------------------------------------------------------
   -- This function return a source/line/column prefix to messages compatible whith GNU Standard
   -- (refer to <https://www.gnu.org/prep/standards/html_node/Errors.html>), that is:
   -- > sourcefile:lineno:
   -- if no column, or
   -- > sourcefile:lineno.column:
   -- otherwise.
   --
   -- Note that there is a trailing space, so that the message can be append directly.
   -- With the first form of the function, the displayed file name is given as an in string parameter.
   -- It is recommanded to use this form for files given on command line, to ease error msg understanding.
   -- For example, a "../../rules.txt" command line parameter should be displayed as is
   -- in case of messages regarding this rules.txt file.
   --
   -- -------------------------------------------------------------------------
   ...
   -- -------------------------------------------------------------------------
   -- Function: GNU_Prefix
   -- With the second form, the file is given as an Ada.Text_IO.File_Type, and
   -- <Ada.Directories.Full_Name> is used to display the file name.
   -- -------------------------------------------------------------------------
